...
|------|------|
| `-n <N>` | 总进程数 |
| `-R "span[ptile=N]"` | 每节点进程数（控制节点分布） |
| `-m <node>` | 指定节点（可选，不指定由调度器自动分配） |
| `-app <app_id>` | 指定应用（如果集群配置了 MPI 应用） |

**节点数 = 总进程数 / 每节点进程数**，例如 `-n 16 -R "span[ptile=4]"` → 4 个节点，每节点 4 进程。
...
### 从 JH_HOSTS 生成 hostfile（通用）

**hostfile 一行表示一个核心（一个 MPI 进程）**，因此需要将每个节点名按 slot 数展开。三种 MPI 实现均适用此格式。
...
echo "$JH_HOSTS" | awk '{for(i=1;i<=$2;i++) print $1; for(j=3;j<=NF;j+=2) for(k=1;k<=$((j+1));k++) print $j}' > "$HOSTFILE"
```
...
```bash
# Intel MPI / MPICH
mpirun -n 16 -machinefile "$HOSTFILE" ./my_program input.dat

# OpenMPI（也支持此格式）
mpirun -np 16 --hostfile "$HOSTFILE" ./my_program input.dat
```
...
mpirun -machinefile "$HOSTFILE" ./my_program input.dat
```
...
jsub -J mpi_job -n 32 -R "span[ptile=8]" \
...
mpiexec -n 32 -machinefile "$HOSTFILE" -binding domain ./my_program input.dat
```
...
```bash
jsub -J mpich_job -n 32 -R "span[ptile=8]" \
...
```bash
jsub -J openmpi_job -n 32 -R "span[ptile=8]" \
  bash -c "module purge && module load openmpi/4.1.5 && mpirun -np 32 --bind-to core ./my_program input.dat"
```
...
| hostfile 格式 | 每行一个节点名（展开 slot 数次） | 每行一个节点名（展开 slot 数次） | 每行一个节点名（展开 slot 数次）或 `hostname slots=N` |
...
Modules 初始化脚本可能未在 shell 配置中加载。手动添加：
...
# 或在脚本开头加一行
eval $(/usr/bin/modulecmd bash load intelmpi/2021.11)
```
