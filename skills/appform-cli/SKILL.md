...
操作 HPC 集群时，按以下流程主动引导用户，不要被动等待指令。

### 1. 提交作业前 — 先确认参数

**禁止直接提交作业。** 必须先用 `--dry-run` 预览参数让用户确认：

```bash
# 第一步：预览参数（不实际提交）
appform jobs submit -a starccm -i /path/test.sim --dry-run

# 用户确认后：去掉 --dry-run 执行提交
appform jobs submit -a starccm -i /path/test.sim -n 32
```

如果用户未指定关键参数（CPU 数、队列、版本等），**必须先询问**再提交，不要自行假设默认值。

### 2. 识别应用类型

用户提到后处理、可视化等需求时，先确认应用类型：
...
| TYPE | 用途 | 操作 |
|------|------|------|
| `batch` | 计算应用 | `appform jobs submit -a <app_id>` |
| `desktop` | 交互应用（如 meta_post、ParaView） | `appform sessions start --app-id <app_id>` |
| 无 TYPE | 管理入口 | 不用于提交或会话 |

**当用户需要后处理/可视化时**，自动查找 `TYPE=desktop` 的应用，引导使用 `sessions start`，而非 `jobs submit`。

搜索应用时，如果关键词未匹配到结果，扩大搜索范围（去掉后缀、缩短关键词、或直接列出全部再筛选）。
...
```bash
# 方式 1：查看单个作业状态
appform jobs get <job_id>                # 查看详情中的 Status 字段

# 方式 2：按状态过滤列表
appform jobs list --status RUN           # 查看运行中的作业
appform jobs list --status PEND          # 查看等待中的作业
appform jobs list --job-id <job_id>      # 按 ID 查询
```

终态：`DONE`（成功）、`EXIT`（失败）、`ZOMBI`（异常）。非终态时需要继续轮询。

### 4. 作业完成后 — 主动引导下一步

作业状态变为 `DONE` 后，主动询问用户下一步操作：
...
# 下载结果文件到本地
appform jobs files <job_id> get /output/result.dat ./
appform jobs files <job_id> get /output/ ./results/

# 启动后处理交互应用（查找 desktop 类型应用）
appform apps list | grep -i post
appform sessions start --app-id meta_post --start-new --work-file '$HOME/results/result.dat'
```

### 5. 直接启动交互应用

用户要求打开可视化/后处理工具时（无需先提交计算任务）：
...
如果 `connect-launch` 失败（JHApp 未安装），将 `jhappUrl` 或 `webUrl` 直接输出给用户，告知可点击访问。
