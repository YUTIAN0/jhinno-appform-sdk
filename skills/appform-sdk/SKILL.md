...
**禁止将密码、密钥、令牌等敏感信息发送给 AI。** 代码中的凭据应由用户自行填写。
...
# 方式 1: AccessKey 认证（推荐，凭据由用户自行填写）
client = AppformClient(
    base_url="<服务器地址>",
    access_key="<用户的 AccessKey>",
    access_key_secret="<用户的 AccessKeySecret>",
    username="<用户名>",
)

# 方式 2: 用户名密码登录
client = AppformClient(base_url="<服务器地址>")
client.auth.login(username="<用户名>", password="<密码>")

# 方式 3: 配置文件（~/.appform/config.json，通过 appform config set 配置）
...
client.files.upload(file_path="local.txt", remote_path="$HOME/simulations/")
