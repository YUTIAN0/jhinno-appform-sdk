...
所有方法均支持 `transfer_method: str = "http"` 参数，可传 `"sftp"` 切换传输协议。

```python
# 列表
client.files.list(path: str = "/", page: int = 1, page_size: int = 100) -> Dict
client.files.list_all(path: str = "/") -> List[Dict]         # 自动分页

# 目录
client.files.mkdir(path: str, force: bool = True) -> Dict

# 复制 / 移动 / 重命名 / 删除
client.files.copy(src_path: str, dest_dir: str) -> Dict
client.files.move(src_path: str, dest_dir: str) -> Dict
client.files.rename(old_path: str, new_name: str) -> Dict
client.files.delete(path: str) -> Dict
